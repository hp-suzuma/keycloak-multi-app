...
- `/` でログイン済みユーザー情報を返却
- `/logout` で Laravel セッションを破棄し、Keycloak のログアウトへ遷移して `https://global.example.com/` に戻る
...
2. Keycloak セッションが破棄される
3. `https://global.example.com/` に戻る
4. 再度ログイン時に Keycloak の認証画面が表示されることを確認する

## 確認済み結果

- `a.example.com` では `app_role: "bff-a"` と `alice / tenant-user-a` を返す
- `b.example.com` では `app_role: "bff-b"` と `bob / tenant-user-b` を返す
- `https://a.example.com/logout` は Keycloak の end-session endpoint に 302 し、`post_logout_redirect_uri=https://global.example.com/` を含む
- `https://global.example.com/logout` は Keycloak の end-session endpoint に 302 し、`post_logout_redirect_uri=https://global.example.com/` を含む
...
- Nuxt と Laravel は最小構成です。実案件では CSRF、エラーハンドリング、監視を補ってください
